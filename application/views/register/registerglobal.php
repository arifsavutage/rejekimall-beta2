<div class="row fullscreen" data-equalizer="foo" style="background:transparent;padding:0px;margin:0px;border:0px;">

  <div class="medium-2 columns" style="background:transparent;padding:0px;margin:0px;border:0px;">
    <div class="hide-for-small" data-equalizer-watch="foo" >
        <div class="row fullscreen" data-equalizer="bar" style="margin:0px;padding:0px;">
        <?php
		$this->load->view('leftbar/leftbarcategory');
		$this->load->view('leftbar/leftbarsupport');
        ?>
        </div>      
    </div>
    </div>

    <div class="medium-8 columns" style="background:transparent;border:0px;" >
    <div class="" data-equalizer-watch="foo">

      <div class="row fullscreen" style="margin:0px;padding:25px 0 0 0px;">
          <div class="large-12 medium-12 small-12 columns" >
            <h3>Pendaftaran Penjual Global</h3>
          </div>
          
      </div> 

      <div class="row fullscreen" style="padding-top:15px;margin:0px;">
          <div class="large-12 medium-12 small-12 columns">
            Silakan lengkapi form dibawah ini untuk mendaftar sebagai Penjual Perorangan.
          <form name="" id="" enctype="multipart/form-data" action="" method="post">
           <div class="row">
            <div class="large-7 medium-12 small-12 columns">
              <label>Username
                <input type="text" placeholder="" required/>
              </label>
            </div>            
          </div>          
          <div class="row">
            <div class="large-7 medium-12 small-12 columns">
              <label>Nama Lengkap
                <text placeholder="" required/></text>
              </label>
            </div>            
          </div>
          <div class="row">
            <div class="large-7 medium-12 small-12 columns">
              <label>Email
                <input type="text" placeholder="" />
              </label>
            </div>            
          </div>
          <div class="row">
            <div class="large-7 medium-12 small-12 columns">
              <label>Kata Sandi
                <input type="text" placeholder="" />
              </label>
            </div>            
          </div>
          <div class="row">
            <div class="large-7 medium-12 small-12 columns">
              <label>Ulangi Kata Sandi
                <input type="text" placeholder="" />
              </label>
            </div>            
          </div>
          <div class="row">
            <div class="large-7 medium-12 small-12 columns">
              <label>Nama Toko
                <text placeholder=""></text>
              </label>
            </div>            
          </div>
          <div class="row">
            <div class="large-7 medium-12 small-12 columns">
              <label>Telepon
                <text placeholder=""></text>
              </label>
            </div>            
          </div>
          <div class="row">
            <div class="large-7 medium-12 small-12 columns">
              <label>Alamat
                <textarea placeholder=""></textarea>
              </label>
            </div>            
          </div>
          <div class="row">
            <div class="large-7 medium-12 small-12 columns">
              <label>Provinsi
                <select>
                  <option value="">...</option>
                  <option value="">...</option>
                  <option value="">...</option>
                  <option value="">...</option>
                </select>
              </label>
            </div>            
          </div>
          <div class="row">
            <div class="large-7 medium-12 small-12 columns">
              <label>Kota
                <select>
                  <option value="">...</option>
                  <option value="">...</option>
                  <option value="">...</option>
                  <option value="">...</option>
                </select>
              </label>
            </div>            
          </div>
          <div class="row">
            <div class="large-7 medium-12 small-12 columns">
              <label>Negara
                <input type="text" placeholder="" />
              </label>
            </div>            
          </div>
          <div class="row">
            <div class="large-7 medium-12 small-12 columns">
              <label>Referensi
                <input type="text" placeholder="Rejekimall" />
              </label>
            </div>            
          </div>
          <div class="row">
            <div class="large-7 medium-12 small-12 columns">
              <label>Nama Bank
                <input type="text" placeholder="" />
              </label>
            </div>            
          </div>
          <div class="row">
            <div class="large-7 medium-12 small-12 columns">
              <label>Nama Pemilik Rekening
                <input type="text" placeholder="" />
              </label>
            </div>            
          </div>
          <div class="row">
            <div class="large-7 medium-12 small-12 columns">
              <label>Nomor Rekening
                <input type="text" placeholder="" />
              </label>
            </div>            
          </div>
          <div class="row">
            <div class="large-7 medium-12 small-12 columns">
              <label>Beneficiary name 
                <input type="text" placeholder="" />
              </label>
            </div>            
          </div>
          <div class="row">
            <div class="large-7 medium-12 small-12 columns">
              <label>Branch name 
                <input type="text" placeholder="" />
              </label>
            </div>            
          </div>
          <div class="row">
            <div class="large-7 medium-12 small-12 columns">
              <label>Switch code 
                <input type="text" placeholder="" />
              </label>
            </div>            
          </div><br>
           <div class="row">
            <div class="large-7 medium-12 small-12 columns">
              <label>Negara
                <input type="text" placeholder="" />
              </label>
            </div>            
          </div>
          <div class="row">
            <div class="large-7 medium-12 small-12 columns">
              <label>Alamat Bank
                <input type="text" placeholder="" />
              </label>
            </div>            
          </div>
          <div class="row">
            <div class="large-7 medium-12 small-12 columns">
              <label>Kode Post
                <input type="text" placeholder="" />
              </label>
            </div>            
          </div>
          <div class="row">
            <div class="large-7 medium-12 small-12 columns">
              <label>Kota / Provinsi / Kabupaten
                <input type="text" placeholder="" />
              </label>
            </div>            
          </div>
          <div class="row">
            <div class="large-7 medium-12 small-12 columns">
              <label>Tax Registration
                <input type="file" placeholder="" />
              </label>
            </div>            
          </div>
          <div class="row">
            <div class="large-7 medium-12 small-12 columns">
              <label>USD Bank Account Passbook
                <input type="file" placeholder="" />
              </label>
            </div>            
          </div>
          <div class="row">
            <div class="large-7 medium-12 small-12 columns">
              <label>Berlangganan Newsletter</label> 
               <input type="radio" id="newsletter" name="newsletter" value="Setuju" checked="checked"><label for="Setuju">Setuju</label>
               <input type="radio" id="newsletter" name="newsletter" value="tSetuju">  <label for="tSetuju">Tidak Setuju</label>                         
           </div>            
         </div>
         <div class="row">
            <div class="large-7 medium-12 small-12 columns">
              <label>Berlangganan SMS</label> 
               <input type="radio" id="SMS" name="SMS" value="Setuju" checked="checked"><label for="Setuju">Setuju</label>
               <input type="radio" id="SMS" name="SMS" value="tSetuju">  <label for="tSetuju">Tidak Setuju</label>                         
           </div>            
         </div>
         <div class="row">
            <div class="large-7 medium-12 small-12 columns">
              <label>Kode Cpatcha
                <br><img src="captcha.php" width="100" height="50" />
                <input type="text" placeholder="" />
              </label>
            </div>            
          </div>
           <div class="row">
            <div class="large-12 medium-12 small-12 columns">             
               <input type="radio" name="" value="" id=""><label>Saya Telah Setuju dengan <a href="">Syarat dan Ketentuan</a> dari RejekiMall</label>             
            </div>            
          </div>
          <div class="row">
            <div class="large-7 medium-12 small-12 columns">             
                <input type="submit" placeholder="" class="alert button"/>             
            </div>            
          </div>


          </form>
          </div>
        </div>      
   </div>
   </div>

   <div class="medium-2 columns" style="background:transparent;padding:0px;margin:0px;border-top:0px;border-bottom:0px;border-right:0px;border-left:solid 0.1em #EFEFEF;">
    <div class="" data-equalizer-watch="foo">
      <div class="row fullscreen" style="padding:0px;margin:0px;">
       <?php
	   $this->load->view('rightbar/rightbarproduct');
       ?>
     </div>
     <div class="row fullscreen" style="padding:0px;margin:0px;">
       <?php
	   $this->load->view('rightbar/rightbarbanner');
       ?>
     </div>  
   </div>
 </div>
</div>